...
//===========================================================================
 const getAllNames = arr => {
   let getNames = arr.map(user => (user.name));
   return getNames;
 };
...
const getUsersByEyeColor = (arr, color) => {
  const getUsersByEye = arr.filter(function (user) {
    if (user.eyeColor === color) {
      return user;
    }
  })
  return getUsersByEye.map(user => (`объект ${user.name}`));
};
...
const getUsersByGender = (arr, gender) => {
  const findUsersByEye = arr.filter(function (user) {
    if (user.gender === gender) {
      return user.gender;
    }
  })
  return findUsersByEye.map(user => (user.name));
};

console.log(getUsersByGender(users, 'male')); // [ 'Moore Hensley', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson' ]
//=============================================================================
const getInactiveUsers = arr => {
  const findInactiveUsers = arr.filter(function (user) {
    if (!user.isActive) {
      return user;
    }
  })
  return findInactiveUsers.map (user => (user));
};
//
...
const getUsersWithAge = (arr, min, max) => {
  const isValidAge = arr.filter(user => user.age > min && user.age < max)
  return isValidAge;
};
...
const getTotalBalance = arr => {
    const calculateBalance = arr.reduce((acc,user) => acc + user.balance, 0);
...
const getUsersByFriend = (arr, name) => {
  const findUsersByFriend = arr.filter(function (user) {
    if (user.friends.includes(name)) {
      return user;
    }
  });
  return findUsersByFriend.map (user => (user.name));
};
